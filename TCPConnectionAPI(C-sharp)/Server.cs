            ...
            using (IClientAbilityProtocol clientProtocol = new ClientAbilityProtocol())
            {
                while (true)
                {
                    switch (_protocol.ReceiveCommand(user.ConnectionSocket))
                    {
                        ...
                        case CommandsToServer.GetAllDetailNomenclatures:
                            {
                                _protocol.SendCollection(clientProtocol.FindDetailNomenclaturesWhere(c => c != null), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.PreviousRoom:
                            {
        ...
        protected virtual void AdminHandler(ConnectedUserInfo user)
        {
            ...
            using (IAdminAbilityProtocol adminProtocol = new AdminAbilityProtocol())
            {
                while (true)
                {
                    switch (_protocol.ReceiveCommand(user.ConnectionSocket))
                    {
                        case CommandsToServer.CreateDetailNomenculature:
                            {
                                if (adminProtocol.CreateDetailNomenclature(_protocol.ReceiveObject<DetailNomenclature>(user.ConnectionSocket)))
                                    _protocol.SendAnswerFromServer(AnswerFromServer.Successfully, user.ConnectionSocket);
                                else
                                    _protocol.SendAnswerFromServer(AnswerFromServer.Error, user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.ModifyDetailNomenclature:
                            {
                                if (adminProtocol.ModifyDetailNomenclature(_protocol.ReceiveObject<DetailNomenclature>(user.ConnectionSocket)))
                                    _protocol.SendAnswerFromServer(AnswerFromServer.Successfully, user.ConnectionSocket);
                                else
                                    _protocol.SendAnswerFromServer(AnswerFromServer.Error, user.ConnectionSocket);
                                break;
                        ...
                        case CommandsToServer.DeleteDetailNomenclature:
                            {
                                int id = int.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                if (adminProtocol.DeleteDetailNomenclaturesWhere(c => c.Id == id))
                                    _protocol.SendAnswerFromServer(AnswerFromServer.Successfully, user.ConnectionSocket);
                                else
                                    _protocol.SendAnswerFromServer(AnswerFromServer.Error, user.ConnectionSocket);
                                break;
                        ...
                        case CommandsToServer.GetAllDetailNomenclatures:
                            {
                                _protocol.SendCollection(adminProtocol.FindDetailNomenclaturesWhere(c => c != null), user.ConnectionSocket);
                                break;
                            }
            ...
            using (IExpertAbilityProtocol expertProtocol = new ExpertAbilityProtocol())
            {
                while (true)
                {
                    switch (_protocol.ReceiveCommand(user.ConnectionSocket))
                    {
                        ...
                        case CommandsToServer.GetAllDetailNomenclatures:
                            {
                                _protocol.SendCollection(expertProtocol.FindDetailNomenclaturesWhere(c => c != null), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.RateDetailNomenclature:
                            {
                                ...
                                var buf = expertProtocol.FindDetailNomenclaturesWhere(c => c.Id == entityId)[0];
