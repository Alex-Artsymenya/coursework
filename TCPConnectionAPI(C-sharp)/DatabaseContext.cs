...
namespace TCPConnectionAPI_C_sharp_
{
    class DatabaseContext : IMainDBPermission
    {
        ...
        virtual public int CreateDetailNomenclature(DetailNomenclature obj)
        {
            DBconnection.Configuration.ProxyCreationEnabled = true;
            DBconnection.DetailNomenclatureSet.Add(obj);
            DBconnection.SaveChanges();
            DBconnection.Configuration.ProxyCreationEnabled = false;
            return obj.Id;
        }
        ...
        virtual public bool DeleteDetailNomenclature(Func<DetailNomenclature, bool> comparer)
        {
            var buf = FindDetailNomenclaturesWhere(comparer);
            if (buf.Count == 0) { return false; }
            else
            {
                DBconnection.Configuration.ProxyCreationEnabled = true;
                DBconnection.DetailNomenclatureSet.RemoveRange(buf);
                DBconnection.Configuration.ProxyCreationEnabled = false;
                DBconnection.SaveChanges();
                return true;
            }
        ...
        virtual public List<DetailNomenclature> FindDetailNomenclaturesWhere(Func<DetailNomenclature, bool> comparer)
        {
            return DBconnection.DetailNomenclatureSet
                .Include("RateSet")
                .Where(comparer)
                .ToList();
        }

        virtual public bool UpdateDetailNomenclature(DetailNomenclature newVersion)
        {
            DBconnection.Configuration.ProxyCreationEnabled = true;
            var buf = DBconnection.DetailNomenclatureSet.Find(newVersion.Id);
            ...
            else
            {
                ...
                buf.Description = newVersion.Description;
                buf.DetailType = newVersion.DetailType;
                DBconnection.SaveChanges();
                DBconnection.Configuration.ProxyCreationEnabled = false;
                return true;
            }
