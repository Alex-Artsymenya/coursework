...
namespace TCPConnectionAPI_C_sharp_
{
    class DatabaseContext
    {
        ...
        virtual public int CreateVehicle(Vehicle newVehicle)
        {
            DBconnection.Configuration.ProxyCreationEnabled = true;
            DBconnection.VehicleSet.Add(newVehicle);
            DBconnection.SaveChanges();
            DBconnection.Configuration.ProxyCreationEnabled = false;
        ...
        virtual public bool DeleteVehiclesWhere(Func<Vehicle, bool> comparer)
        {
            var vehicleSet = FindVehiclesWhere(comparer);
            if (vehicleSet.Count == 0) { return false; }
            else
            {
                DBconnection.Configuration.ProxyCreationEnabled = true;
                DBconnection.VehicleSet.RemoveRange(vehicleSet);
                DBconnection.Configuration.ProxyCreationEnabled = false;
                DBconnection.SaveChanges();
                return true;
            }
