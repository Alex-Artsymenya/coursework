...
namespace TCPConnectionAPIClientModule_C_sharp_
{
    public class ClientConnectionModule : IAdminAccess, IExpertAccess, IClientAccess
    {
        ...
        public AnswerFromServer CreateDetailNomenclature(DetailNomenclature obj)
        {
            protocol.SendCommand(CommandsToServer.CreateDetailNomenculature);
            protocol.SendObject(obj);
            return protocol.ReceiveAnswerFromServer();
        }

        public AnswerFromServer ModifyDetailNomenclature(DetailNomenclature newVersion)
        {
            protocol.SendCommand(CommandsToServer.ModifyDetailNomenclature);
            protocol.SendObject(newVersion);
            return protocol.ReceiveAnswerFromServer();
        }

        public AnswerFromServer DeleteDetailNomenclature(int id)
        {
            protocol.SendCommand(CommandsToServer.DeleteDetailNomenclature);
        ...
        public List<DetailNomenclature> GetAllDetailNomenclatures()
        {
            protocol.SendCommand(CommandsToServer.GetAllDetailNomenclatures);
            return protocol.ReceiveCollection<DetailNomenclature>();
        }
        ...
        public AnswerFromServer RateDetailNomenclature(int entityId, float expertRate)
        {
            protocol.SendCommand(CommandsToServer.RateDetailNomenclature);
